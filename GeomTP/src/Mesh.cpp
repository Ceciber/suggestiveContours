...
void Mesh::addPlan(float square_half_side)
{
    ...
    _triangleIndices.push_back(glm::uvec3(_vertexPositions.size() - 4, _vertexPositions.size() - 3, _vertexPositions.size() - 2));
    _triangleIndices.push_back(glm::uvec3(_vertexPositions.size() - 4, _vertexPositions.size() - 2, _vertexPositions.size() - 1));
...
void Mesh::init()
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, _curvatureVbo);
  glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 0, (void*)0); // Attribute 3 for curvature (2 floats per vertex)

  glEnableVertexAttribArray(4);
  glBindBuffer(GL_ARRAY_BUFFER, _pDir1Vbo);
  glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, 0, (void*)0); // Attribute 4 for principalDir1 (3 floats per vertex)
  ...
  /* to only visualize contours disable the curvature related code
