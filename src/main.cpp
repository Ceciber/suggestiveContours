...
struct Scene {
  ...
  {
    if (!rhino) {
        std::cerr << "[Error] Rhino mesh is NULL, cannot render!" << std::endl;
        return;
    }

    std::cout << " > Rendering Rhino mesh..." << std::endl;
    ...
    mainShader->set("materialAlbedo", glm::vec3(1, 0.71, 0.29));
  ...
  void applyTaubin(){
    rhino->computeNeighboringVertices();
    rhino->taubinSmooth(0.33f, -0.34f, 5);
    rhino->init();
  }
...
void initOpenGL()
{
  ...
  std::cerr << "New shaders loaded" << std::endl;
  std::string vertexSource = loadShaderSource("src/line.vert");
  std::string fragmentSource = loadShaderSource("src/line.frag");
...
GLuint lineShaderProgramID = createShaderProgram(vertexSource, fragmentSource);
if (lineShaderProgramID == 0) {
    std::cerr << "Failed to create line shader program!" << std::endl;
    return;
}

// Store the shader program ID in your Mesh class or somewhere accessible
g_scene.rhino->setLineShaderProgramID(lineShaderProgramID);
}

void initScene(const std::string &meshFilename)
{
  std::cout << " > initScene() called with file: " << meshFilename << std::endl;
  // Init camera
  int width, height;
